FreiChat.init_HTML_freichatX = function() {
    ...
    str_opt1 += "    <div class='frei_status_options'> \n\
...
<img id='frei_img' src=" + FreiChat.make_url(freidefines.minimg) + " alt='max' height=12 width=12/> </span></div>";
    str_frei = "<div id='frei_user_brand' class='frei_user_brand'>\n\
...
\n\
    ...
    main_str = str_contain + str_extras + "        <div class='' id='frei_chatroom_notify'>\n\
...
<div id='freichat' class='freichat' style='z-index: 99999;'>" + str_options + str_head + str_frei + str_off + "</div>";
...
\n\
    ...
    FreiChat.freiopt = $jn("#frei_options");
    FreiChat.mainchat = $jn("#freichat");
    FreiChat.frei_minmax_img = $jn("#frei_img");
    FreiChat.freiOnOffline = $jn("#onfreioffline");
    FreiChat.datadiv = $jn("#FREICHATXDATASTORAGE");
    FreiChat.custom_mesg_div = $jn("#custom_status_change");
    FreiChat.freicontain = [$jn('.freicontain0'), $jn('.freicontain1'), $jn('.freicontain2'), $jn('.freicontain3')];
    FreiChat.Date = new Date();
    if (freidefines.PLUGINS.showchatroom == 'enabled') {
        FreiChat.chatroom = $jn('#frei_chatroom');
        FreiChat.roomcontainer = $jn('#frei_roomcontainer');
    }
    if (freidefines.GEN.rtl == '1') {
        $jn("#freichat_rtl_img").attr('src', FreiChat.make_url(freidefines.rtlimg_enabled));
    } else {
    ...
    if (freidefines.GEN.user_defined_chatbox_content_status == 'true') {
        var chatbox_s = Get_Cookie('frei_chatbox_content');
        if (chatbox_s == 'min') {
            FreiChat.chatbox_container.hide();
        } else {
            FreiChat.chatbox_container.show();
        }
    }
    FreiChat.freiopt.hide();
    FreiChat.freiOnOffline.hide();
    FreiChat.option_bar_sound_bg = $jn(".frei_option_bar_sound_bg");
    FreiChat.toggle_sound(false);
    if (FreiChat.chatbox_container.is(":visible") == true) {
...
FreiChat.toggle_sound = function(clicked) {
    ...
    if (sound == cond) {
        Set_Cookie('frei_sound', 'off');
        FreiChat.option_bar_sound_bg.addClass('frei_option_bar_sound_bg_off');
        if (FreiChat.option_bar_sound_bg.hasClass('frei_option_bar_sound_bg_on')) {
            FreiChat.option_bar_sound_bg.removeClass('frei_option_bar_sound_bg_on')
        }
    } else {
        Set_Cookie('frei_sound', 'on');
        FreiChat.option_bar_sound_bg.addClass('frei_option_bar_sound_bg_on');
        if (FreiChat.option_bar_sound_bg.hasClass('frei_option_bar_sound_bg_off')) {
...
FreiChat.init_process_freichatX = function() {
    ...
    if (cond1 || cond2) {
        FreiChat.private_chat = "disabled";
    } else {
        FreiChat.private_chat = "enabled";
    }
    var status = FreiChat.util.storage.get("frei_mystatus");
    if (!status) {
        FreiChat.util.storage.set("frei_mystatus", FreiChat.freistatus);
    } else {
        FreiChat.freistatus = status;
    }
    ...
    if (freidefines.GEN.is_guest === "0" || freidefines.SET.allow_guest_name_change === 'no') {
        $jn('#frei_custom_guest_name').hide();
...
FreiChat.min_max_freichat = function(min) {
    ...
    if (FreiChat.chatbox_container.is(":visible") == false && min != 'min') {
        FreiChat.frei_minmax_img.attr('src', FreiChat.make_url(freidefines.minimg));
        FreiChat.chatbox_container.slideDown();
        Set_Cookie('frei_chatbox_content', 'max');
    } else {
        FreiChat.frei_minmax_img.attr('src', FreiChat.make_url(freidefines.maximg));
        FreiChat.chatbox_container.slideUp();
...
FreiChat.analyse = function() {
    ...
    var status = '';
    if (FreiChat.ses_status == 1) {
        status = freidefines.STATUS.TEXT.online;
    } else if (FreiChat.ses_status == 2) {
        status = freidefines.STATUS.TEXT.invisible;
    } else if (FreiChat.ses_status > 2) {
        status = freidefines.STATUS.TEXT.busy;
    }
    $jn("#frei_option_bar_status_txt").html(status);
    $jn.getJSON(freidefines.GEN.url + "server/freichat.php?freimode=getdata", {
        ...
        var i, language, from_name, idfrom, divToappend, uniqueid, users_length, last_chatmessage_usr_id, user, id, reidfrom, message, CookieStatus;
        last_chatmessage_usr_id = i = 0;
        for (i = 0; i < message_length; i++) {
            user = id = null;
            reidfrom = freidefines.GEN.reidfrom;
            var participants = data.messages[i].group_participants,
                gid = data.messages[i].room_id,
                wid, talkingTo, g_ids = [],
                g_users = [];
            var is_gmesg = data.messages[i].message_type === '3';
            ...
            if (is_gmesg) {
                ...
                while (len--) {
                    g_ids.push(participants[len].id);
                    g_users.push(participants[len].username);
                }
                talkingTo = g_users.join();
            } else {
                wid = id;
                talkingTo = user;
            }
            message = data.messages[i].message;
            CookieStatus = FreiChat.getCookie(wid);
            if (CookieStatus.chatwindow_1 == "opened") {
                var box_count = FreiChat.create_chat_window(talkingTo, wid);
                if (is_gmesg && !$jn('#chatboxtextarea' + wid).data('data').messageType) {
                    var _data = {
                        user: g_users,
                        id: g_ids,
                        gid: gid,
                    ...
                    $jn('#chatboxtextarea' + wid).data('data', _data);
                }
                if (box_count !== -1)
                    box_counts.push(box_count);
                message = FreiChat.SmileyGenerate(message, wid);
                language = CookieStatus.language;
                from_name = data.messages[i].from_name;
                idfrom = data.messages[i].from;
                divToappend = $jn("#chatboxcontent_" + wid + " .frei_content");
                uniqueid = FreiChat.unique++;
                var mesg_html = FreiChat.generate_mesg(uniqueid, data.messages[i], message, wid);
                divToappend.append(mesg_html);
                FreiChat.setCookie("frei_stat_" + wid, CookieStatus.language + "&opened&" + CookieStatus.chatwindow_2 + "&nclear&" + CookieStatus.pos_top + "&" + CookieStatus.pos_left);
            }
        }
        ...
        if (CookieStatus.chatwindow_1 == "opened") {
            ...
            for (i = 0; i <= users_length; i++) {
                if (freichatusers[i] == undefined || freichatusers[i] == 0) {
                    ...
                    var gid = $jn('#chatboxtextarea' + freichatusers[i]).data('data').gid;
                    var id = (typeof gid === 'undefined') ? freichatusers[i] : "g_" + gid;
                    FreiChat.toggleChatBoxOnLoad(id, box_counts[i]);
                    FreiChat.scroll_down("chatboxcontent_" + id, freichatusers[i]);
                }
            }
        }
    }, 'json');
...
FreiChat.create_chat_window_mesg = function(user, id) {
    ...
    if (FreiChat.RequestCompleted_isset_mesg == true) {
        ...
        $jn.getJSON(freidefines.GEN.url + "server/freichat.php?freimode=isset_mesg", {
            ...
            var message_length = data.messages.length;
            var j = 0;
            var idto, idfrom, reidfrom, message, from_name, divToappend, uniqueid, language, last_chatmessage_usr_id;
            last_chatmessage_usr_id = 0;
            for (j = 0; j < message_length; j++) {
                ...
                if (idfrom == reidfrom && idto == id || idfrom == id && reidfrom == idto) {
                    ...
                    uniqueid = FreiChat.unique++;
                    language = CookieStatus.language;
                    var mesg_html = FreiChat.generate_mesg(uniqueid, data.messages[j], message, id);
                    divToappend.append(mesg_html);
                }
            }
...
FreiChat.message_append = function(messages) {
    ...
    var i, j, exist, userlen, user, id, message, CookieStatus, fromname, newtitle, canPass, from_name, language, divToappend, uniqueid, toid;
    i = j = 0;
    for (i = 0; i < message_length; i++) {
        exist = false;
        userlen = freichatusers.length;
        var participants = messages[i].group_participants,
            gid = messages[i].room_id,
            wid, talkingTo, g_ids = [],
            g_users = [],
            is_gmesg = messages[i].message_type === '3';
        if (is_gmesg) {
            wid = "g_" + gid;
            var len = participants.length;
            while (len--) {
            ...
            wid = messages[i].from;
            talkingTo = messages[i].from_name;
        }
        for (j = 0; j < userlen; j++) {
        ...
        user = messages[i].from_name;
        id = messages[i].from;
        toid = messages[i].to;
        message = messages[i].message;
        if (exist == false) {
            freichatusers.push(wid);
            FreiChat.chatWindowHTML(talkingTo, wid);
        }
        if (is_gmesg) {
            var data = {
                user: g_users,
            ...
            $jn('#chatboxtextarea' + wid).data('data', data);
        }
        message = FreiChat.SmileyGenerate(message, wid);
        CookieStatus = FreiChat.getCookie(wid);
        fromname = user;
        newtitle = freidefines.newmesg + " " + fromname;
        canPass = false;
        if (message != '') {
            var timeOut = 0;
            ...
            if (canPass == true) {
                var change_title = function() {
                    ...
                    $jn('#chatboxhead' + wid).data('interval', 'true');
                    if (FreiChat.change_titletimer == null) {
                        FreiChat.change_titletimer = setInterval(change_title, 2000);
                    }
                };
                change_title();
                $jn('#chatboxhead' + wid).css('background-image', 'url(' + FreiChat.make_url(freidefines.newtopimg) + ')');
                FreiChat.API.beep();
            }
        }
        from_name = fromname;
        ...
        divToappend = $jn("#chatboxcontent_" + wid + " .frei_content");
        uniqueid = FreiChat.unique++;
        var mesg_html = FreiChat.generate_mesg(uniqueid, messages[i], message, wid);
        divToappend.append(mesg_html);
        FreiChat.setCookie("frei_stat_" + wid, CookieStatus.language + "&opened&max&nclear&0&0");
        FreiChat.scroll_down("chatboxcontent_" + wid, wid);
    }
};
...
FreiChat.search_members = function(user_data) {
    ...
    if (search_value !== "") {
        ...
        for (var u = 0; u < user_arr_len; u++) {
            curr_user = user_data[u].username.toLowerCase();
            if (curr_user.indexOf(search_value) > -1) {
                userdata.push(user_data[u]);
            }
        }
    } else {
        userdata = user_data;
    ...
    if ((FreiChat.inactive == false && FreiChat.freistatus != 3) || FreiChat.temporary_status > 10 || FreiChat.private_chat == "disabled") {
        ...
        if (FreiChat.RequestCompleted_get_members == true) {
            ...
            $jn.getJSON(freidefines.GEN.url + "server/freichat.php?freimode=getmembers", {
                ...
                if (FreiChat.name_exists(FreiChat.custom_gst_name) && freidefines.GEN.is_guest === "1") {
                    $jn('#custom_guest_name_id').val(FreiChat.mod_guest_name($jn('#custom_guest_name_id').val(), true));
                    FreiChat.update_custom_gst_name();
                }
                if (FreiChat.custom_gst_name != freidefines.GEN.fromname && freidefines.GEN.is_guest === "1") {
                    freidefines.GEN.fromname = FreiChat.custom_gst_name;
                } else {
                    freidefines.GEN.fromname = data.username;
                }
                freidefines.GEN.reidfrom = data.userid;
                freidefines.GEN.is_guest = data.is_guest;
                FreiChat.setCookie('frei_time', data.time);
                FreiChat.setCookie('frei_custom_mesg', FreiChat.custom_mesg);
                FreiChat.long_poll = 'true';
                ...
                for (j = 0; j < userlen; j++) {
                    if (FreiChat.is_in_array(freichatusers[j], data.userdata) === false && !$jn('#chatboxtextarea' + freichatusers[j]).data('data')) {
                        $jn('#frei_chat_status_' + freichatusers[j]).show().html(freidefines.TRANS.chat_status);
                    } else {
                        $jn('#frei_chat_status_' + freichatusers[j]).hide();
                    }
                }
                var userdata_len = data.userdata.length;
                for (var i = 0; i < userdata_len; i++) {
                    if (!FreiChat.is_in_array(data.userdata[i].userid, FreiChat.userdata)) {
                        FreiChat.API.onUser(data.userdata[i], !FreiChat.first);
                    }
                }
                if (data.count == 0) {
                    FreiChat.divfrei.css("height", freidefines.fnoonlineht);
                } else if (data.count == 1) {
                    FreiChat.divfrei.css("height", freidefines.fone_onlineht);
                } else if (data.count > 1 && data.count < 5) {
                    FreiChat.height = data.count * 27;
                    FreiChat.divfrei.css("height", FreiChat.height);
                } else if (data.count > 5) {
                    FreiChat.divfrei.css("height", freidefines.fmaxht);
                }
                if (freidefines.PLUGINS.showchatroom == 'enabled') {
                    var old_room_online_count = FreiChat.room_online_count;
                    FreiChat.room_online_count = data.room_online_count;
                    if (FreiChat.first == false) {
                        FreiChat.room_array = data.room_array;
                        FreiChat.roomcreator(1);
                        if (!FreiChat.is_allowed('CHATROOM_CRT')) {
                            $jn('#frei_create_chatroom').hide();
                        }
                    } else {
                        var a_len = data.room_array.length;
                        for (var i = 0; i < a_len; i++) {
                            FreiChat.room_array.push(data.room_array[i]);
                        }
                        if (FreiChat.room_array.length != FreiChat.room_online_count.length) {
                            FreiChat.modify_room_array("FreiChat");
                        } else {
                            if (a_len) {
                                FreiChat.roomcreator();
                            } else {
                                FreiChat.update_room_online_cnt(old_room_online_count, FreiChat.room_online_count, FreiChat.room_array);
                            }
                        }
                    }
                    if (FreiChat.in_room == "-1") {
                        if (!$jn("#frei_lobby").is(":visible"))
                            FreiChat.load_lobby();
                    } else {
                        if (!$jn('#dc-slick-9').hasClass('active') && FreiChat.first && data.chatroom_messages.length > 0) {
                            FreiChat.chatroom_notify("", data.chatroom_messages);
                        }
                        ...
                        if (found && FreiChat.first == false) {
                            FreiChat.loadchatroom(FreiChat.room_array[index].room_name, FreiChat.in_room, FreiChat.room_array[index].room_type);
                        }
                        var room, ai;
                        ai = room = 0;
                        for (ai = 0; ai < data.chatroom_messages.length; ai++) {
                            if (data.chatroom_messages[ai].room_id >= 0) {
                                room = data.chatroom_messages[ai].room_id;
                                FreiChat.chatroom_written[room] = true;
                            }
                        }
                ...
                if (data.islog == "guesthasnopermissions") {
                    FreiChat.divfrei.css("height", freidefines.fnopermsht).html(freidefines.nopermsmesg);
                ...
                if (data.userdata.length > 0) {
                    userdata = FreiChat.search_members(data.userdata);
                }
...
FreiChat.create_users = function(userdata) {
    ...
    if (!(userdata.length > 0 && userdata[0].username))
        return userdata;
    var users_html = "";
    var users_len = userdata.length - 1;
    var show_avatar = freidefines.SET.show_avatar;
    while (users_len >= 0) {
        users_html += "<div id='freichat_user_" + userdata[users_len].userid + "' title='" + userdata[users_len].status_mesg + "' \n\
...
FreiChat.on_enter_press = function(event, chatboxtextarea, id, user, option, box_count) {
    ...
    FreiChat.scroll_down(freiarea.attr("id"), id, false);
    message = message.replace(/^\s+|\s+$/g, "");
    if (option == 0) {
        freiarea.css('height', '44px');
    }
    if (message != '') {
        message = FreiChat.formatMessage(message, id);
        ...
        if (option == 0) {
            ...
            var content_div = $jn("#chatboxcontent_" + id + " .frei_content");
            var data = {
                from: freidefines.GEN.fromid,
                from_name: freidefines.GEN.fromname,
                GMT_time: 0
            }
            var mesg_html = FreiChat.generate_mesg(uniqueid, data, message, id);
            content_div.append(mesg_html);
            FreiChat.scroll_down("chatboxcontent_" + id, id);
        } else {
            ...
            message = FreiChat.plugins.formatter.format(message);
            var message_div = '';
            message_div = '<div id = "' + local_in_room + '_chatroom_message"  class="frei_chatroom_message"><span style="display:none" id="' + local_in_room + '_message_type">LEFT</span>\n\
            ...
            if (freidefines.GEN.reidfrom == FreiChat.last_chatroom_usr_id && FreiChat.chatroom_written[FreiChat.in_room] == true) {
        ...
        setTimeout(function() {
            if (option == 0) {
                ...
                FreiChat.sendMessage(local_in_room, FreiChat.bulkmesg, user, 1);
            }
...
FreiChat.chatWindowHTML = function(user, id) {
    ...
    while (len--) {
        if (freidefines.GEN.fromname != users[len])
 ...
 <div class="freicontent_' + id + '" id="freicontent_' + id + '"> <div id="chatboxcontent_' + id + '" class="chatboxcontent frei_nanocontent"><div class="frei_groupchat" id="frei_groupchat_' + id + '" ><input type="text" id="frei_groupchat_input_' + id + '" placeholder="Add people to this chat"/><div id="add_groupchat_btn_' + id + '" class="groupchat-done">Add</div></div><div class="frei_content"></div></div>     \n\
    ...
    FreiChat.API.onChatWindow({
        user: user,
        id: id,
        isGroup: isGroup
    });
    $jn('#chatboxtextarea' + id).data('data', {
        id: [_id],
        user: [user],
        cnt: FreiChat.box_count
    }).keyup(function(event) {
        $jn(this).scrollTop($jn(this)[0].scrollHeight);
        if (event.keyCode == 13 && event.shiftKey == 0) {
            FreiChat.on_enter_press(event, this, id, user, 0, $jn(this).data('data').cnt);
        }
    });
    if (FreiChat.is_allowed('GROUPCHAT') && freidefines.PLUGINS.showmobilechat == 'enabled') {
        $jn('.frei_tooltip').tooltipster();
        FreiChat.set_window_title(id);
    }
    $jn('#frei_' + id).bind({
        click: function() {
    ...
    FreiChat.set_drag(id, FreiChat.box_count);
    if (freidefines.SET.addedoptions_visibility === "HIDDEN") {
        $jn('#addedoptions_' + id).hide();
    }
    $jn("#frei_trans" + id).hide();
    $jn('#frei_chat_status_' + id).hide();
    if (freidefines.GEN.content_height !== "auto") {
        var pane = $jn("#chatboxcontent_" + id);
        FreiChat.jscrollers.push("chatboxcontent_" + id);
        pane.nanoScroller({
        ...
        $jn("#chatboxcontent_" + id).css("overflow-y", "scroll").find('.frei_content').css('overflow-y', 'hidden');
        $jn("#chatboxcontent_" + id + " > .frei_content").css({
            position: "static",
            padding: 0
        });
    }
    if ((typeof FreiChat.cached_frei_ht === "undefined")) {
        FreiChat.cached_frei_ht = $jn('#frei_' + id).height();
    }
    return FreiChat.box_count;
};
FreiChat.set_window_title = function(id) {
    if (typeof $jn('#chatboxtextarea' + id).data('data') === 'undefined')
        return false;
    var users = $jn('#chatboxtextarea' + id).data('data').user;
    var len = users.length,
        titles = [];
    while (len--) {
        if (users[len] != freidefines.GEN.fromname) {
            titles.push(users[len]);
        }
    }
    var title = titles.join(",");
    var el = $jn("#frei_window_title_" + id);
    var short_title = title;
    if (title.length > 22) {
        short_title = title.substr(0, 21) + "...";
    }
...
FreiChat.sendMessage = function(id, message, user, type) {
    if (FreiChat.bulkmesg.length >= 1) {
        ...
        if (FreiChat.RequestCompleted_send_messages == true) {
            ...
            if (type === 1) {
                message = FreiChat.plugins.formatter.formatBB(message);
            }
            var jData = $jn('#chatboxtextarea' + id).data('data');
            if (!jData)
                jData = {};
            $jn.post(freidefines.GEN.url + "server/freichat.php?freimode=post", {
                ...
                xhash: freidefines.xhash,
                id: freidefines.GEN.getid,
                to: (jData.id) ? jData.id.join(",") : id,
                chatroom_mesg_time: FreiChat.chatroom_mesg_time,
                message_type: (jData.messageType) ? jData.messageType : type,
                'message[]': [message],
                to_name: (jData.user) ? jData.user.join(",") : user,
                custom_mesg: FreiChat.custom_mesg,
                in_room: (jData.gid) ? jData.gid : in_room,
...
FreiChat.toggleChatBox = function(id, box_count) {
    ...
    var el = FreiChat.freicontain[box_count];
    var div = FreiChat.freicontain[box_count];
    if ($jn("#" + id).is(":visible")) {
        ...
        if (div.hasClass("ui-draggable"))
            div.draggable('disable');
        $jn('#frei_' + idx).css("position", "absolute");
    } else {
        if (freidefines.SET.draggable === 'enable') {
        ...
        $jn("#" + id).show('clip', options, 300, function() {
            ...
            FreiChat.scroll_down("chatboxcontent_" + idx, idx);
...
FreiChat.closeChatBox = function(id, box_pos, empty) {
    ...
    delete FreiChat.last_chatmessage_usr_id[idx];
    var CookieStatus = FreiChat.getCookie(idx);
    FreiChat.setCookie("frei_stat_" + idx, CookieStatus.language + "&closed&max&0&0");
    var options = {};
    $jn("#" + id).hide('explode', options, 1000).remove();
    if (typeof empty === "undefined") {}
    var i = 0,
        users_length = freichatusers.length;
    for (i = 0; i <= users_length; i++) {
...
FreiChat.clrcht = function(id) {
    if (typeof id === "undefined") {} else {
        ...
        if (CookieStatus.message != "clear") {
            ...
            $jn("#chatboxcontent_" + id + " .frei_content").html("<font size='1' color='#A4A4A4'>" + freidefines.chatHistoryDeleted + "</font>");
        } else {
...
FreiChat.freichatopt = function(opt) {
    ...
    var remove = 'false';
    if (FreiChat.freistatus == 1) {
        remove = 'frei_status_available';
    } else if (FreiChat.freistatus == 2) {
        remove = 'frei_status_invisible';
    } else if (FreiChat.freistatus > 2) {
        remove = 'frei_status_busy';
    }
    if (remove != 'false') {
        if (opt == "nooptions")
            $jn('#' + remove).addClass("frei_status_options_selected");
        else
    ...
    if (opt == "nooptions") {
        ...
        $jn('#frei_status_available').addClass("frei_status_options_selected");
        $jn("#frei_option_bar_status_txt").html(freidefines.STATUS.TEXT.online);
        FreiChat.freistatus = 1;
        if (FreiChat.freiopt.is(":visible") != false) {
        ...
        FreiChat.freiopt.slideToggle();
        $jn('#frei_status_invisible').addClass("frei_status_options_selected");
        $jn("#frei_option_bar_status_txt").html(freidefines.STATUS.TEXT.invisible);
        FreiChat.freistatus = 2;
    } else if (opt == "goBusy") {
        FreiChat.freiopt.slideToggle();
        $jn('#frei_status_busy').addClass("frei_status_options_selected");
        $jn("#frei_option_bar_status_txt").html(freidefines.STATUS.TEXT.busy);
...
FreiChat.getCookie = function(id) {
    ...
    if (stat_str == false || typeof stat_str == "undefined" || stat_str == null) {
...
FreiChat.show_plugins = function(user, id) {
    ...
    if (FreiChat.plugins.is_allowed("FORMATTER") && is_chatroom) {
        pluginhtml += FreiChat.plugins.formatter.get_html();
    }
    ...
    pluginhtml += '<span id="save' + id + '"><a href="javascript:void(0)" onClick="FreiChat.analyse();"><img class="frei_added_options_img" id="load' + id + '" src="' + FreiChat.make_url(freidefines.toolimg) + '" title="Load chats" alt="load chats" /> </a></span>';
    return pluginhtml;
};
FreiChat.attach_document_events = function() {
    $jn(document).mousemove(function() {
        ...
        var cnt = $jn("#frei_chatroom_cp");
        if (cnt.has(e.target).length === 0) {
            cnt.hide();
        }
    });
};
...
FreiChat.remove_from_array = function(element, array) {
...
FreiChat.selfInvoke = function(jQuery) {
    if (freidefines.GEN.custom_error_handling == 'YES') {
        ...
        addHandler(window, 'onerror', function(msg) {
            ...
            if (typeof freidefines.freichat_error_report == "undefined") {
                var style = "<style>body{background:#efefef;}.freichat_error_report {min-height: 20px;padding: 19px;margin-bottom: 20px;background-color: #f5f5f5;border: 1px solid rgba(0, 0, 0, 0.05);border-radius: 4px;box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.05);}</style>";
                var desc = "<div class='freichat_error_report'><b>BOOT:</b> " + freidefines.GEN.BOOT + "<br/><b>jQuery loaded forcefully:</b> " + freidefines.GEN.force_load_jquery + "</div> "
                freidefines.freichat_error_report = style + "<h2 style='text-align:center;'>FREICHAT ERROR REPORT</h2><br/>" + desc + "<br/>";
            }
            freidefines.freichat_error_report += error;
            return true;
        });
    }
    ...
    if (X_init == false) {
        ...
        $jn(window).load(function() {
            soundManager.onready(function() {
                if (soundManager.supported()) {
                    FreiChat.beep = soundManager.createSound({
                        id: 'mySound',
                        url: freidefines.GEN.url + "client/jquery/img/newmsg.mp3"
                    });
                } else {
            ...
            FreiChat.attach_document_events();
...
FreiChat.init_chatrooms = function() {
    ...
    FreiChat.chatroom.dcSlick({
        ...
        align: 'left',
        offset: freidefines.PLUGINS.chatroom_offset,
        speed: 'slow',
        classTab: 'frei_tab',
    ...
    $jn("#frei_chatroom_notify").css(freidefines.PLUGINS.chatroom_location, "0").css("margin-" + freidefines.PLUGINS.chatroom_location, "4px");
    ...
    if (freidefines.PLUGINS.chatroom_rotate != "0") {
        ...
        if (ie_ver === -1 || (ie_ver !== -1 && ie_ver > 8.0)) {
    ...
    frei_tab.css(position_shift, freidefines.PLUGINS.chatroom_label_offset);
...
FreiChat.loadchatroom = function(title, id, type, me) {
    ...
    FreiChat.chatroom_changed = true;
    FreiChat.in_room = id;
...
FreiChat.append_chatroom_message_div = function(messages, type) {
    ...
    for (i = 0; i < message_length; i++) {
        ...
        if (messages[i].from == FreiChat.last_chatroom_usr_id && FreiChat.chatroom_written[FreiChat.in_room] == true) {
